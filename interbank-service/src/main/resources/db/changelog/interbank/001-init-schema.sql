...
-- changeset interbank:32-001-init

-- ============================================================================
-- PR_32 Phase 1: inter-bank protokol — pocetna sema (4 tabele).
--
-- Reference: docs/superpowers/specs/2026-05-11-pr32-interbank-protocol-design.md §4.
--
-- Tabele:
...
-- Idempotency lookup: vraca outbound poruke koje su PENDING_SEND ili SENT-bez-confirma
-- i jos uvek imaju retries left (5 attempts max). Partial index drzi index mali.
...
-- Lookup poruka po transaction ID (debug + 2PC correlation).
...
-- "Hung 2PC" scanner: PREPARED transakcije starije od X sek se auto-abort-uju.
-- Partial index, samo PREPARED je relevantno.
...
-- Lookup negotiation-a po buyer-u (kupac vidi sve ponude koje je pokrenuo / dobio).
CREATE INDEX idx_interbank_negotiations_remote
    ON interbank_negotiations(buyer_routing_number, buyer_id);

-- Settlement scheduler: nadje ongoing negotiation-e ciji je settlement_date prosao
-- i auto-finalizuje ih ili otpise. Partial index — samo ongoing relevantno.
...
-- Expiry job: pronadji ACTIVE kontrakte cije je settlement_date prosao i flip-uj
-- na EXPIRED. Plus za buyer-side dohvat "moji aktivni ugovori".
CREATE INDEX idx_interbank_contracts_status_settle
    ON interbank_contracts(status, settlement_date);

-- Reserved-stock invariant query: koliko akcija seller drzi rezervisano u
-- aktivnim kontraktima za dati ticker (anti-double-sell guard).
